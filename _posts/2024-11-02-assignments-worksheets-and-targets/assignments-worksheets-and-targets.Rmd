...
1. figuring out how to set up the solutions so that they can be rendered or not
1. setting up a worksheet with several scenarios and their questions and solutions
1. creating two versions of the rendered document, one with solutions and one without
1. making all this happen in Targets, respecting the dependency of each worksheet on its constituent scenario files (and maybe datafiles as well, if they are likely to change).
...
The other notable feature here is how I label the two questions: the `(@)` on the front, which will auto-number them from 1 upwards in the final worksheet.^[I would actually prefer to have these questions numbered 1(a) and 1(b), but I haven't figured out how to control this numbering in the way you can in LaTeX.]
...
This, I have to admit, I stole more or less wholesale from [Nicola Rennie](https://nrennie.rbind.io/blog/r-tutorial-worksheets-quarto/), whose blog post you would do well to read. There are two key ideas:

- [Parameterised documents](https://quarto.org/docs/computations/parameters.html#knitr)
...
![](Screenshot from 2024-11-02 00-52-36.png)

at the top, and

![](Screenshot from 2024-11-02 00-54-27.png)
...
Now we have the machinery to add some optionally-displayable text, that is to say, solutions, to our problems. What you do is to add the code that optionally starts the div block at the *start* of a solution, and the code that optionally ends it at the end. Thus, for example, the Motor Trend question file with solutions^[Much briefer than my usual solutions, it has to be said.] looks like this:

![](Screenshot from 2024-11-02 00-49-51.png)
...
![](Screenshot from 2024-11-02 21-21-14.png)
...
I wrote this just *before* reading [Danielle Navarro's excellent blog post](https://blog.djnavarro.net/posts/2024-10-06_fs/) on the `fs` package, and now I realize that this would have been a great reason to learn about that package. I had, however, gotten this working using `SplitPath` from the `DescTools` package, so this is what you get. Also, I realize, now that I look at the code, that it would have benefitted greatly from using `glue::glue` rather than `str_c` from `stringr`.^[Which is really just `paste0`.]
...
I hope in this way I am safe from [having my computer set on fire](https://github.com/jennybc/here_here), although I could undoubtedly stand to learn about the `here` package too.
...
The (very brief) idea behind `targets` is that certain of your files (like documents) depend on certain other things (included files, here, or functions or datasets in general). The [Targets book](https://books.ropensci.org/targets/walkthrough.html) has a great intro walkthrough. What you do is to create "targets", and then have functions that express how targets depend on each other. The definition of the targets lives in a file `_targets.R` in the project folder. Here is the relevant bit of that:
...
My function `renderify` is in a file `R/functions.R` which has been `source`d earlier in `_targets.R`.
...
- [questions file](https://ritsokiguess.site/datafiles/worksheet_99_q.html)
- [solutions file](https://ritsokiguess.site/datafiles/worksheet_99_a.html)
