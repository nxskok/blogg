...
The simulated data values are called `z` and the groups are called `x` and `y` in column `gp`.

Now we generate some data and run the two flavours of $t$-test on it:
...
```{r}
d1 <- gen_sample(c(10, 10), c(100, 140), c(30, 30))
t.test(z ~ gp, data = d1)
